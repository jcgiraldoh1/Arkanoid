...
extern Sprite *speedBallsSprites[];
...
PowerUpEntity::PowerUpEntity()
{
	...
	powerUpType = PowerUpType::SPEEDBALLS;

	for(int i = 0; i < 6; ++i)
		spriteList.push_back(multiBallSprites[i]);
	
	for (int i = 0; i < 6; ++i)
		spriteList.push_back(speedBallsSprites[i]);
...
PowerUpEntity * MakePowerUp(PowerUpType type, float xpos, float ypos)
{
	...
	case PowerUpType::SPEEDBALLS:
		...
		for (int i = 0; i < 6; ++i)
			powerUpEntity->spriteList[i] = speedBallsSprites[i];
		powerUpEntity->sprite = powerUpEntity->spriteList[0];
